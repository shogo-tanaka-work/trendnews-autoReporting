...
cp -r "$SRC/systemd/tech-radar-collect@trend_digest.service.d" "$UNIT_DIR/"
...
systemctl enable --now tech-radar-connpass.timer
...
次の確認:
  ...
  journalctl -u tech-radar-collect@tech_cloud --since '5 min ago' --no-pager
EOS
if [[ "$api_ok" -eq 1 ]]; then
