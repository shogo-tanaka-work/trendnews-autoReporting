...
5. 全カテゴリの `tech-radar-collect@<category>.timer` と `tech-radar-connpass.timer`・`tech-radar-keywords.timer` を有効化する。
...
- サービスは**リポジトリの所有ユーザー**として動く。`ProtectSystem=strict` を掛けているので、
  書き込み先は unit の `ReadWritePaths=`（`data/`。neta_weekly は `archive/` と `.git/`、keywords は `archive/`）だけ。
